...
## We see that county and state are in the same column

glimpse(hpsa_mh_data)

## We'll want to separate those into different columns https://tidyr.tidyverse.org/reference/separate.html

hpsa_mh_data_sep_county_state <- hpsa_mh_data %>% 
  separate(county, c("county","filler","state")) %>% 
  dplyr::select(-filler) %>% 
  print()

## Buy when we sort by county we have to account for counties with multiple separate words as their names, but it's actually worse than that. Multiple HPSAs per county

hpsa_mh_data_sep_county_state %>% 
  group_by(county, hpsa_id) %>% 
  tally() %>% 
  print()
